...
const TODAY_TUTORIAL: TutorialStep[] = [
  ...
  { icon: ListChecks, title: "Your command center", body: "The one thing that matters most right now, plus how ready your Passport is — updated live, never guessed." },
 ...
 */
export function TodayCommandCenter({ firstName, signals, onSignalsChanged }: TodayCommandCenterProps) {
  ...
  return (
    <div className="space-y-4">
      <FeaturePageHeader
        ...
        tabs={
          <div className="flex flex-wrap items-center justify-center gap-2.5">
            <KretoPresence state={priority.kind === "none" ? "idle" : "attentive"} size="compact" className="shrink-0" />
            <button
              type="button"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,220px)_1fr] gap-3">
        {passport && (
          <HoloCard className="rounded-2xl border border-border bg-card p-4 flex flex-col justify-between">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Passport readiness</p>
              <p className="text-3xl font-black leading-none mt-1.5">{passport.percentage}%</p>
            </div>
            <div className="mt-3">
              <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                <div
                  className="h-full rounded-full bg-primary transition-all"
                  style={{ width: `${passport.percentage}%` }}
                />
              </div>
              {!passport.isComplete && passport.missingFields[0] && (
                <p className="text-[11px] text-muted-foreground mt-2 line-clamp-1">
                  Next: {passport.missingFields[0]}
                </p>
              )}
            </div>
          </HoloCard>
        ...
        <div className="rounded-2xl border border-primary/30 bg-primary/[0.03] p-4 flex flex-col">
          <AnimatePresence mode="wait">
            {priority.kind === "task" ? (
              <motion.div key={priority.task.id} initial={reducedMotion ? false : { opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="flex-1">
              ...
              <motion.div key={priority.approval.id} initial={reducedMotion ? false : { opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="flex-1">
              ...
              <motion.div key={priority.gig.id} initial={reducedMotion ? false : { opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="flex-1">
              ...
              <motion.div key="empty" initial={reducedMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex items-center gap-3">
          ...
          <CtaButton onClick={runPrimaryCta} className="w-full sm:w-auto mt-3 gap-1.5">
